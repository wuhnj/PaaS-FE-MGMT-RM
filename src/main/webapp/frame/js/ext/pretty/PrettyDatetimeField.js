...
Ext.ux.PrettyTimePickerField = function(config){
	if(CU.isEmpty(config)) config = {};
	Ext.ux.PrettyTimePickerField.superclass.constructor.apply(this, arguments);
};
Ext.extend(Ext.ux.PrettyTimePickerField, Ext.form.Field, {
	...
	onRender: function(ct, position){
        Ext.ux.PrettyTimePickerField.superclass.onRender.call(this, ct, position);
        this.rendered=false;
  		this.date=new Date();
  		var values={};
  		if(this.value) {
	  	...
	  	var cfg=Ext.apply({}, this.spinnerCfg, {
			...
			listeners: {
			spin: {fn: this.onSpinnerChange, scope: this},
		    valid: {fn: this.onSpinnerChange,scope: this},
	...
	getRawValue: function() {
	  if(!this.hoursSpinner){
	   ...
	   return {h: this.hoursSpinner.getValue(),m: this.minutesSpinner.getValue(),s: this.secondsSpinner.getValue()};
	  }
	},
	...
	getValue: function() {
	  ...
	  return String.leftPad(v.h, 2, '0')+':'+String.leftPad(v.m, 2, '0')+':'+String.leftPad(v.s, 2, '0');
	},
...
Ext.form.PrettyTimePickerField=Ext.ux.PrettyTimePickerField;

Ext.ux.DateTimePicker = function(config){
	if(CU.isEmpty(config)) config = {};
	Ext.ux.DateTimePicker.superclass.constructor.apply(this, arguments);
};
Ext.extend(Ext.ux.DateTimePicker, Ext.DatePicker, {
	timeFormat:'g:i:s A',
	timeLabel:'时间',
	timeWidth:100,
	initComponent:function() {Ext.ux.DateTimePicker.superclass.initComponent.call(this);this.id = Ext.id();},
	onRender: function(container,position){
        ...
        var table = Ext.get(Ext.DomQuery.selectNode('table tbody',container.dom));
       	Ext.DomHelper.insertBefore(table.last(),{tag:'tr',
            children:[{tag:'td',cls:'x-date-bottom x-form-item x-form-item-label',html:this.timeLabel,style:'width:30;'},{tag:'td',cls:'x-date-bottom ux-timefield', colspan:'2'}]},true);
    ...
    setValue : function(value){
        if (!this.tf){
            this.tf = new Ext.ux.PrettyTimePickerField();
...
Ext.ux.PrettyDateTimeField = function(config){
	if(CU.isEmpty(config)) config = {};
	Ext.ux.PrettyDateTimeField.superclass.constructor.apply(this, arguments);
};
Ext.extend(Ext.ux.PrettyDateTimeField, Ext.ux.PrettyDateField, {
	dateFormat: 'Y-m-d',
	timeFormat: 'H:i:s',
	defaultAutoCreate : {tag: "input", type: "text", size: "20", autocomplete: "off"},
	initComponent:function() {
        Ext.ux.PrettyDateTimeField.superclass.initComponent.call(this);
    ...
    getValue : function(){
    	...
    	return CU.isEmpty(value) ? "" : CU.toDate(value);
    },
    isValid : function() {
    	return this.autoValid!==false ? true : Ext.ux.PrettyDateTimeField.superclass.isValid.call(this);
    },
    validate: function() {
    	return this.autoValid!==false ? true : Ext.ux.PrettyDateTimeField.superclass.validate.call(this);
    },
