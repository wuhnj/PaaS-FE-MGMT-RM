 ...
 * @xtype itemselector 
 * 
 * add leftall rightall at 2010-02-03
 */
Ext.ux.form.ItemSelector = Ext.extend(Ext.form.Field,  {
    ...
    drawUpIcon:true,
    drawDownIcon:true,
    drawLeftAllIcon:true,
    drawLeftIcon:true,
    drawRightIcon:true,
    drawRightAllIcon:true,
    drawTopIcon:true,
    drawBotIcon:true,
    delimiter:',',
    bodyStyle:null,
    ...
    onRender: function(ct, position){
        ...
        this.iconRightAll = this.imagePath + (this.iconRightAll || 'rightall.gif');
        ...
        el.createChild({tag: 'br'});
        this.rightAllIcon = el.createChild({tag:'img', src:this.iconRightAll, style:{cursor:'pointer', margin:'2px'}});
        el.createChild({tag: 'br'});
        ...
        el.createChild({tag: 'br'});
        this.leftAllIcon = el.createChild({tag:'img', src:this.iconLeftAll, style:{cursor:'pointer', margin:'2px'}});
        el.createChild({tag: 'br'});
        ...
        this.leftAllIcon.on('click', this.leftAll, this);
        this.rightAllIcon.on('click', this.rightAll, this);
        if (!this.drawUpIcon || this.hideNavIcons) { this.upIcon.dom.style.display='none'; }
        if (!this.drawDownIcon || this.hideNavIcons) { this.downIcon.dom.style.display='none'; }
        if (!this.drawLeftIcon || this.hideNavIcons) { this.addIcon.dom.style.display='none'; }
        if (!this.drawLeftAllIcon || this.hideNavIcons) { this.leftAllIcon.dom.style.display='none'; }
        if (!this.drawRightIcon || this.hideNavIcons) { this.removeIcon.dom.style.display='none'; }
        if (!this.drawRightAllIcon || this.hideNavIcons) { this.rightAllIcon.dom.style.display='none'; }
    ...
    leftAll: function() {
    	var formstore = this.toMultiselect.view.getStore() ;
		formstore.each(function(record){this.fromMultiselect.view.store.add(record);},this);
     	formstore.removeAll();
    },
    rightAll: function() {
    	var formstore = this.fromMultiselect.view.getStore() ;
		formstore.each(function(record){this.toMultiselect.view.store.add(record);},this);
     	formstore.removeAll();
    },
