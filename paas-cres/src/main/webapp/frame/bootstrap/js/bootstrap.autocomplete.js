!function($){
  ...
  * ================================= */
  ...
  , select: function () {
      ...
      var r = this.hide();
      if(CU.isFunction(this.options.onSelect))this.options.onSelect(r);
      return r;
    }
  ...
  , lookup: function (event) {
      ...
      if ((!this.query&&!this.options.emptyQuery)  || this.query.length < this.options.minLength) {
  ...
  , listen: function () {
      this.$element
        ...
        .on('click',	$.proxy(this.elclick, this))
  ...
  , getDropValue : function(dropDef) {
	  var name = this.$element.val();
	  if(CU.isEmpty(name)) return "";
	  var item = CU.getDropItemRecordByName(dropDef, name);
	  return CU.isEmpty(item) ? -1 : item.code;
  }
  }
